...
ax.plot(forecast_df["Fecha"], forecast_df["IPC_Proyectado"], color="#457B9D",
        linewidth=2, linestyle="--", label="LSTM Forecast")
ax.set_title("Mexico CPI Forecast to 2030 — LSTM Model (MAPE: 1.86%)", fontsize=14)
ax.set_xlabel("Year")
ax.set_ylabel("CPI Index")
ax.legend()
ax.grid(alpha=0.3)
plt.tight_layout()
plt.savefig("../results/lstm_forecast_2030.png", dpi=150)
plt.show()
print("Plot saved to results/lstm_forecast_2030.png")
